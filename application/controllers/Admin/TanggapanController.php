...
class TanggapanController extends CI_Controller {
	...
	public function tanggapan_detail($id = null)
	{
		$id = $id ? $id : htmlspecialchars($this->input->post('id',true));
		...
		if ( ! empty($cek_data)) :
			...
			if ($this->form_validation->run() == FALSE) :
				...
				if ($update_status_pengaduan) :
					...
					$masyarakat = $this->db->get_where('masyarakat',['nik' => $pengaduan['nik']])->row_array();
					$this->load->model('Notifikasi_m');
					$this->Notifikasi_m->create([
						'id_pengaduan'  => $id_pengaduan,
						'dari_username'  => $this->session->userdata('username'),
						'untuk_username' => $masyarakat['username'],
						'isi'            => 'Pengaduan Anda telah selesai diproses',
						'url'            => 'Masyarakat/PengaduanController/pengaduan_detail/' . $id_pengaduan,
	...
	public function tambah_tanggapan()
	{
		...
		if ( ! empty($cek_data)) :
			...
			$tanggapan_existing = $this->db->get_where('tanggapan',['id_pengaduan' => $id_pengaduan])->row_array();
			if ($tanggapan_existing) :
				$this->session->set_flashdata('msg','<div class="alert alert-danger" role="alert">
					Pengaduan ini sudah ditanggapi sebelumnya!
					</div>');
			...
			$this->form_validation->set_rules('id', 'id', 'trim|required');
			$this->form_validation->set_rules('status', 'Status Pengaduan', 'trim|required|in_list[proses,tolak]');
			...
			if ($this->form_validation->run() == FALSE) :
				...
				if ($menanggapi) :
					...
					if ($update_status_pengaduan) :
						...
						$this->load->model('Notifikasi_m');
						$status = htmlspecialchars($this->input->post('status',true));
						$pesan = ($status == 'proses') ? 'sedang diproses' : 'ditolak';
						$this->Notifikasi_m->create([
							'id_pengaduan'  => $id_pengaduan,
							'dari_username'  => $this->session->userdata('username'),
							'untuk_username' => $masyarakat['username'],
							'isi'            => 'Pengaduan Anda ' . $pesan,
							'url'            => 'Masyarakat/PengaduanController/pengaduan_detail/' . $id_pengaduan,
							'created_at'     => date('Y-m-d H:i:s'),
						]);
